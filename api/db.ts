import prisma from "@/lib/prisma";
import {
  CreateMovieData,
  CreateSeriesData,
  Movie,
  Series as SeriesType,
  WatchedMovie,
} from "@/types";

type MovieWithWatchStatsRecord = Movie & {
  watch_events: Array<{
    id: number;
    watched_at: Date;
  }>;
  _count: {
    watch_events: number;
  };
};

type MovieWatchEventRecord = {
  id: number;
  watched_at: Date;
  movie: Movie & {
    _count: {
      watch_events: number;
    };
  };
};

function mapMovieWithWatchStats(movie: MovieWithWatchStatsRecord): Movie {
  const latestWatch = movie.watch_events[0] ?? null;

  return {
    ...movie,
    watched_at: latestWatch?.watched_at ?? null,
    watch_count: movie._count.watch_events,
  };
}

function mapWatchedMovie(event: MovieWatchEventRecord): WatchedMovie {
  return {
    ...event.movie,
    watch_event_id: event.id,
    watched_at: event.watched_at,
    watch_count: event.movie._count.watch_events,
  };
}

async function getMovieWithWatchStatsByWhere(
  userId: string,
  where: {
    id?: number;
    tmdb_id?: number;
  }
): Promise<Movie | null> {
  const movie = (await prisma.movies.findFirst({
    where: {
      userId,
      ...where,
    },
    include: {
      watch_events: {
        select: {
          id: true,
          watched_at: true,
        },
        orderBy: {
          watched_at: "desc",
        },
        take: 1,
      },
      _count: {
        select: {
          watch_events: true,
        },
      },
    },
  })) as MovieWithWatchStatsRecord | null;

  return movie ? mapMovieWithWatchStats(movie) : null;
}

async function requireOwnedMovieId(userId: string, id: number): Promise<number> {
  const movie = await prisma.movies.findFirst({
    where: {
      id,
      userId,
    },
    select: {
      id: true,
    },
  });

  if (!movie) {
    throw new Error("Movie not found");
  }

  return movie.id;
}

async function requireOwnedSeriesId(userId: string, id: number): Promise<number> {
  const series = await prisma.series.findFirst({
    where: {
      id,
      userId,
    },
    select: {
      id: true,
    },
  });

  if (!series) {
    throw new Error("Series not found");
  }

  return series.id;
}

export async function getLatestWatchedMovies(
  userId: string,
  limit: number = 10,
  offset: number = 0
): Promise<WatchedMovie[]> {
  const events = (await prisma.movie_watch_events.findMany({
    where: {
      userId,
    },
    take: limit,
    skip: offset,
    orderBy: [{ watched_at: "desc" }, { id: "desc" }],
    include: {
      movie: {
        include: {
          _count: {
            select: {
              watch_events: true,
            },
          },
        },
      },
    },
  })) as MovieWatchEventRecord[];

  return events.map(mapWatchedMovie);
}

export async function getRecentlyAddedMovies(
  userId: string,
  limit: number = 10,
  offset: number = 0
): Promise<Movie[]> {
  const movies = (await prisma.movies.findMany({
    where: {
      userId,
      watch_events: {
        none: {},
      },
    },
    take: limit,
    skip: offset,
    orderBy: {
      created_at: "desc",
    },
    include: {
      watch_events: {
        select: {
          id: true,
          watched_at: true,
        },
        orderBy: {
          watched_at: "desc",
        },
        take: 1,
      },
      _count: {
        select: {
          watch_events: true,
        },
      },
    },
  })) as MovieWithWatchStatsRecord[];

  return movies.map(mapMovieWithWatchStats);
}

export async function getMovie(userId: string, id: number): Promise<Movie | null> {
  return getMovieWithWatchStatsByWhere(userId, { id });
}

export async function addMovieToList(
  userId: string,
  movieData: CreateMovieData
): Promise<Movie> {
  const movie = (await prisma.movies.create({
    data: {
      ...movieData,
      userId,
    },
    include: {
      watch_events: {
        select: {
          id: true,
          watched_at: true,
        },
        orderBy: {
          watched_at: "desc",
        },
        take: 1,
      },
      _count: {
        select: {
          watch_events: true,
        },
      },
    },
  })) as MovieWithWatchStatsRecord;

  return mapMovieWithWatchStats(movie);
}

export async function removeMovieFromList(
  userId: string,
  id: number
): Promise<Movie> {
  const movie = await getMovie(userId, id);

  if (!movie) {
    throw new Error("Movie not found");
  }

  await prisma.movies.delete({
    where: {
      id,
    },
  });

  return movie;
}

export async function movieExistsInDb(
  userId: string,
  tmdb_id: number
): Promise<number | false> {
  const movie = await prisma.movies.findFirst({
    where: {
      userId,
      tmdb_id,
    },
    select: {
      id: true,
    },
  });

  return movie ? movie.id : false;
}

type ExistingMovieData = {
  id: number;
};

type NewMovieData = {
  id: number;
  title: string;
  overview: string | null;
  release_date: Date | null;
  runtime: number | null;
  genres: string[] | null;
  poster_path: string | null;
  imdb_id: string | null;
};

export async function markMovieAsWatched<T extends boolean>(
  userId: string,
  movieData: T extends true ? ExistingMovieData : NewMovieData,
  now: Date,
  isMovieInDb: T
): Promise<Movie> {
  if (isMovieInDb) {
    const movieId = await requireOwnedMovieId(
      userId,
      (movieData as ExistingMovieData).id
    );

    await prisma.$transaction([
      prisma.movies.update({
        where: { id: movieId },
        data: {
          watched_at: now,
          updated_at: now,
        },
      }),
      prisma.movie_watch_events.create({
        data: {
          movie_id: movieId,
          userId,
          watched_at: now,
        },
      }),
    ]);

    return (await getMovie(userId, movieId)) as Movie;
  }

  const mappedMovieData = {
    tmdb_id: (movieData as NewMovieData).id,
    imdb_id: (movieData as NewMovieData).imdb_id,
    title: (movieData as NewMovieData).title,
    overview: (movieData as NewMovieData).overview || null,
    release_date: (movieData as NewMovieData).release_date,
    runtime: (movieData as NewMovieData).runtime || null,
    genres: (movieData as NewMovieData).genres || [],
    poster_url: (movieData as NewMovieData).poster_path
      ? `https://image.tmdb.org/t/p/original${(movieData as NewMovieData).poster_path}`
      : null,
    watched_at: now,
    updated_at: now,
  };

  const createdMovieId = await prisma.$transaction(async (tx) => {
    const movie = await tx.movies.upsert({
      where: {
        userId_tmdb_id: {
          userId,
          tmdb_id: mappedMovieData.tmdb_id,
        },
      },
      update: mappedMovieData,
      create: {
        ...mappedMovieData,
        created_at: now,
        userId,
      },
      select: {
        id: true,
      },
    });

    await tx.movie_watch_events.create({
      data: {
        movie_id: movie.id,
        userId,
        watched_at: now,
      },
    });

    return movie.id;
  });

  return (await getMovie(userId, createdMovieId)) as Movie;
}

export async function updateMovieScore(
  userId: string,
  movieId: number,
  score: number
): Promise<Movie> {
  const ownedMovieId = await requireOwnedMovieId(userId, movieId);

  await prisma.movies.update({
    where: { id: ownedMovieId },
    data: { score },
  });

  return (await getMovie(userId, ownedMovieId)) as Movie;
}

export async function getMovieByTmdbId(
  userId: string,
  tmdb_id: number
): Promise<Movie | null> {
  return getMovieWithWatchStatsByWhere(userId, { tmdb_id });
}

export async function addSeriesToList(
  userId: string,
  seriesData: CreateSeriesData
): Promise<SeriesType> {
  return (await prisma.series.create({
    data: {
      ...seriesData,
      userId,
    },
  })) as SeriesType;
}

export async function getSeriesByTmdbId(
  userId: string,
  tmdb_id: number
): Promise<SeriesType | null> {
  return (await prisma.series.findFirst({
    where: {
      userId,
      tmdb_id,
    },
  })) as SeriesType | null;
}

export async function removeSeriesFromList(
  userId: string,
  id: number
): Promise<SeriesType> {
  const existing = await prisma.series.findFirst({
    where: {
      id,
      userId,
    },
  });

  if (!existing) {
    throw new Error("Series not found");
  }

  await prisma.series.delete({
    where: {
      id,
    },
  });

  return existing as SeriesType;
}

type ExistingSeriesData = {
  id: number;
};

type NewSeriesData = {
  id: number;
  name: string;
  overview: string | null;
  first_air_date: Date | null;
  last_air_date: Date | null;
  number_of_episodes: number | null;
  number_of_seasons: number | null;
  genres: string[] | null;
  poster_path: string | null;
};

export async function markSeriesAsWatched<T extends boolean>(
  userId: string,
  seriesData: T extends true ? ExistingSeriesData : NewSeriesData,
  now: Date,
  isSeriesInDb: T
) {
  if (isSeriesInDb) {
    return (await prisma.series.update({
      where: {
        id: await requireOwnedSeriesId(userId, (seriesData as ExistingSeriesData).id),
      },
      data: {
        watched_at: now,
        updated_at: now,
      },
    })) as SeriesType;
  }

  const mappedSeriesData = {
    tmdb_id: (seriesData as NewSeriesData).id,
    name: (seriesData as NewSeriesData).name,
    overview: (seriesData as NewSeriesData).overview || null,
    first_air_date: (seriesData as NewSeriesData).first_air_date,
    last_air_date: (seriesData as NewSeriesData).last_air_date,
    number_of_episodes: (seriesData as NewSeriesData).number_of_episodes,
    number_of_seasons: (seriesData as NewSeriesData).number_of_seasons,
    genres: (seriesData as NewSeriesData).genres || [],
    poster_url: (seriesData as NewSeriesData).poster_path
      ? `https://image.tmdb.org/t/p/original${(seriesData as NewSeriesData).poster_path}`
      : null,
    watched_at: now,
    updated_at: now,
  };

  return (await prisma.series.upsert({
    where: {
      userId_tmdb_id: {
        userId,
        tmdb_id: mappedSeriesData.tmdb_id,
      },
    },
    update: mappedSeriesData,
    create: {
      ...mappedSeriesData,
      created_at: now,
      userId,
    },
  })) as SeriesType;
}

export async function updateSeriesScore(
  userId: string,
  seriesId: number,
  score: number
): Promise<SeriesType> {
  const ownedSeriesId = await requireOwnedSeriesId(userId, seriesId);

  return (await prisma.series.update({
    where: { id: ownedSeriesId },
    data: { score },
  })) as SeriesType;
}

export async function getTotalWatchedCount(userId: string): Promise<number> {
  return prisma.movie_watch_events.count({
    where: {
      userId,
    },
  });
}

export async function getMoviesWatchedThisYearCount(
  userId: string
): Promise<number> {
  const now = new Date();
  const startOfYearUTC = new Date(Date.UTC(now.getUTCFullYear(), 0, 1, 0, 0, 0));
  const startOfNextYearUTC = new Date(
    Date.UTC(now.getUTCFullYear() + 1, 0, 1, 0, 0, 0)
  );

  return prisma.movie_watch_events.count({
    where: {
      userId,
      watched_at: {
        gte: startOfYearUTC,
        lt: startOfNextYearUTC,
      },
    },
  });
}

export async function getMoviesWatchedThisMonthCount(
  userId: string
): Promise<number> {
  const now = new Date();
  const startOfMonthUTC = new Date(
    Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1, 0, 0, 0)
  );
  const startOfNextMonthUTC = new Date(
    Date.UTC(now.getUTCFullYear(), now.getUTCMonth() + 1, 1, 0, 0, 0)
  );

  return prisma.movie_watch_events.count({
    where: {
      userId,
      watched_at: {
        gte: startOfMonthUTC,
        lt: startOfNextMonthUTC,
      },
    },
  });
}

export async function getFavoriteGenres(
  userId: string,
  limit: number = 3
): Promise<Array<{ genre: string; count: number }>> {
  const watchedWithGenres = await prisma.movie_watch_events.findMany({
    where: {
      userId,
    },
    select: {
      movie: {
        select: {
          genres: true,
        },
      },
    },
  });

  const genreToCount: Record<string, number> = {};
  for (const item of watchedWithGenres) {
    const genres = (item.movie.genres || []) as string[];
    for (const genre of genres) {
      const key = genre.trim();
      if (!key) continue;
      genreToCount[key] = (genreToCount[key] || 0) + 1;
    }
  }

  return Object.entries(genreToCount)
    .sort((a, b) => b[1] - a[1])
    .slice(0, limit)
    .map(([genre, count]) => ({ genre, count }));
}

function toUTCDateString(date: Date): string {
  const y = date.getUTCFullYear();
  const m = String(date.getUTCMonth() + 1).padStart(2, "0");
  const d = String(date.getUTCDate()).padStart(2, "0");
  return `${y}-${m}-${d}`;
}

export async function getWatchingStreak(userId: string): Promise<{
  currentStreakDays: number;
  longestStreakDays: number;
  lastWatchedAt: Date | null;
  watchedDaysThisYear: number;
}> {
  const watched = await prisma.movie_watch_events.findMany({
    where: {
      userId,
    },
    select: {
      watched_at: true,
    },
    orderBy: { watched_at: "asc" },
  });

  if (watched.length === 0) {
    return {
      currentStreakDays: 0,
      longestStreakDays: 0,
      lastWatchedAt: null,
      watchedDaysThisYear: 0,
    };
  }

  const daySet = new Set<string>();
  for (const watchEvent of watched) {
    daySet.add(toUTCDateString(watchEvent.watched_at));
  }

  const sortedDays = Array.from(daySet)
    .sort()
    .map((day) => new Date(`${day}T00:00:00.000Z`).getTime());

  let longest = 1;
  let current = 1;
  for (let i = 1; i < sortedDays.length; i++) {
    const prev = sortedDays[i - 1];
    const cur = sortedDays[i];
    const oneDay = 24 * 60 * 60 * 1000;
    if (cur - prev === oneDay) {
      current += 1;
      longest = Math.max(longest, current);
    } else {
      current = 1;
    }
  }

  const todayUTC = new Date();
  const todayKey = toUTCDateString(todayUTC);
  const yesterdayUTC = new Date(
    Date.UTC(
      todayUTC.getUTCFullYear(),
      todayUTC.getUTCMonth(),
      todayUTC.getUTCDate() - 1
    )
  );
  const yesterdayKey = toUTCDateString(yesterdayUTC);

  let currentStreak = 0;
  if (daySet.has(todayKey) || daySet.has(yesterdayKey)) {
    let cursor = new Date(
      daySet.has(todayKey)
        ? `${todayKey}T00:00:00.000Z`
        : `${yesterdayKey}T00:00:00.000Z`
    );
    while (daySet.has(toUTCDateString(cursor))) {
      currentStreak += 1;
      cursor = new Date(
        Date.UTC(
          cursor.getUTCFullYear(),
          cursor.getUTCMonth(),
          cursor.getUTCDate() - 1
        )
      );
    }
  }

  const year = todayUTC.getUTCFullYear();
  let watchedThisYear = 0;
  for (const key of daySet) {
    if (key.startsWith(`${year}-`)) watchedThisYear += 1;
  }

  return {
    currentStreakDays: currentStreak,
    longestStreakDays: longest,
    lastWatchedAt: watched[watched.length - 1]?.watched_at ?? null,
    watchedDaysThisYear: watchedThisYear,
  };
}

export async function getRecentActivity(
  userId: string,
  limit: number = 4
): Promise<{
  latestWatched: WatchedMovie[];
  recentlyAdded: Movie[];
}> {
  const [latestWatched, recentlyAdded] = await Promise.all([
    getLatestWatchedMovies(userId, limit),
    getRecentlyAddedMovies(userId, limit),
  ]);

  return { latestWatched, recentlyAdded };
}

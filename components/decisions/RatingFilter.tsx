...
const RatingFilter: React.FC<RatingFilterProps> = ({
  ...
  const presetRatings = [
    { value: 0, label: "Any", description: "Include all" },
    { value: 6.0, label: "Good", description: "Good" },
    { value: 7.0, label: "Great", description: "Strong" },
    {
      value: 8.0,
      label: "Excellent",
      description: "Top",
      ...
      <div className="rounded-[1.25rem] border border-white/10 bg-black/20 p-4">
        <div className="mb-4 rounded-2xl border border-white/8 bg-white/4 p-4 text-center">
          ...
          <div className="text-xs text-zinc-400">
            {minRating === 0
              ? "No floor"
              : `${minRating.toFixed(1)}/10.0 TMDB minimum`}
          </div>
        </div>

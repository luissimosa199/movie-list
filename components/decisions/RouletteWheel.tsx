...
const RouletteWheel: React.FC<RouletteWheelProps> = ({
  ...
  useEffect(() => {
    if (movies.length > 0) {
      const newSegments = generateWheelSegments(movies);
      const newWheelSize = calculateWheelSize(movies.length);
  ...
  useEffect(() => {
    if (winner && !isSpinning) {
      const winnerSegment = segments.find(
        (segment) => segment.movie.id === winner.id
      );
      if (winnerSegment) {
        const segmentElement = document.querySelector(
          `[data-segment-id="${winner.id}"]`
        ) as HTMLElement;
        if (segmentElement) {
          setTimeout(() => {
  ...
  const getSegmentPath = (segment: WheelSegment, radius: number) => {
    const startAngle = (segment.startAngle - 90) * (Math.PI / 180); // -90 to start from top
  ...
  const getTextPosition = (segment: WheelSegment, radius: number) => {
    ...
    const textRadius = radius * 0.8; // Position text closer to edge for better visibility
  ...
  const getPosterPosition = (segment: WheelSegment, radius: number) => {
    const midAngle =
      ((segment.startAngle + segment.endAngle) / 2 - 90) * (Math.PI / 180);
    const posterRadius = radius * 0.6; // Position poster between center and text
      ...
      <div className="relative">
        ...
        >
          <svg
            ...
            style={{ filter: isSpinning ? "blur(0.5px)" : "none" }}
          >
            {/* Wheel Segments */}
            {segments.map((segment) => (
              ...
              >
                ...
                <path
                  ...
                  style={{
                    opacity: winner && winner.id === segment.movie.id ? 1 : 0.9,
                  }}
                />
      ...
      {winner && !isSpinning && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="bg-gradient-to-br from-yellow-500/90 to-orange-500/90 rounded-lg p-4 text-center text-white shadow-lg">
            ...
            <div className="text-sm opacity-90">{winner.title}</div>
          </div>
        </div>
      )}
    </div>
...
export default React.forwardRef<
  ...
  RouletteWheelProps
>((props, ref) => {
  const wheelComponentRef = useRef<any>(null);

  React.useImperativeHandle(ref, () => ({
    spin: (totalRotation: number, duration: number) => {
      if (wheelComponentRef.current) {
        wheelComponentRef.current.spin(totalRotation, duration);
      }
    },
  }));
